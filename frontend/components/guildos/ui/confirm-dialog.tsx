      ...
      <div className="w-full max-w-md rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 shadow-2xl">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            <h2 className="text-base font-semibold text-slate-950 dark:text-white">{req.title}</h2>
            {req.message ? <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">{req.message}</p> : null}
          </div>
        </div>
        ...
        {req.mode === 'prompt' ? (
          <input
            ...
            className="mt-4 w-full rounded-xl border border-slate-200 dark:border-slate-800 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-200"
          />
        ) : null}

        <div className="mt-6 flex justify-end gap-2">
          <button onClick={() => close(req.mode === 'prompt' ? null : false)} className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-4 py-2 text-sm font-medium text-slate-700 dark:text-slate-300 transition hover:bg-slate-50 dark:hover:bg-slate-800">
            {req.cancelLabel}
          </button>
          <button
            onClick={onConfirm}
