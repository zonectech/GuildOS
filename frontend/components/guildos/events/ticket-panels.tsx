    ...
    <div className="w-full space-y-2.5">
      ...
      {quote?.groupDiscount ? (
        quote.discountSource === 'GROUP' ? (
          <p className="text-xs font-semibold text-emerald-700">Group discount applied — each ticket is {quote.groupDiscount.percentOff}% off (₦{quote.listPrice.toLocaleString()} → ₦{quote.price.toLocaleString()})</p>
      ...
      <div className="flex items-center gap-2">
        ...
        />
        <button onClick={() => onApplyPromo(promoInput.trim())} disabled={!promoInput.trim()} className="rounded-xl border border-slate-300 px-3 py-1.5 text-xs font-medium text-slate-700 disabled:opacity-50">Apply</button>
        ...
        {quote?.promo && quote.discountSource === 'GROUP' ? <span className="text-xs text-slate-500">{quote.promo.code} skipped — the group discount is bigger</span> : null}
  ...
  return (
    <section className="rounded-3xl border border-emerald-200 bg-emerald-50 p-5">
      ...
      <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <div><p className="text-xs text-emerald-700">Gross</p><p className="text-lg font-semibold text-emerald-900">₦{sales.grossNgn.toLocaleString()}</p></div>
