...
/** Day-by-day agenda: per-day theme/venue/times/facilitators/sessions/features + day-assigned speakers; cancelled days shown struck-through with the reason. Single-day events render as a plain "Agenda". */
export function EventAgenda({ event, daySpeakers, hasTracks = false, viewerSectionKey = '' }: { event: EventSummary; daySpeakers: Record<number, EventSpeaker[]>; hasTracks?: boolean; viewerSectionKey?: string }) {
  const singleDay = (event.days ?? []).length === 1;
  const sectionName = (key?: string) => (event.sections ?? []).find((s) => s.key === key)?.name ?? '';
  const viewerTrack = sectionName(viewerSectionKey);
  return (
    <section className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 shadow-sm">
      <h2 className="text-lg font-semibold text-slate-950 dark:text-white">{singleDay ? 'Agenda' : 'Day-by-day agenda'}</h2>
      {hasTracks ? (
        <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
          {viewerTrack
            ? `Showing the shared programme plus your ${viewerTrack} track sessions.`
            : 'Shared sessions are for every track; track-tagged sessions run in parallel — pick a section to see just your programme.'}
        </p>
      ) : null}
      {event.theme ? (
      ...
      <ol className="mt-4 space-y-4">
        {(event.days ?? []).map((day, i) => (
          <li key={i} className={`rounded-2xl border p-4 ${day.cancelled ? 'border-rose-200 bg-rose-50/60 dark:border-rose-900 dark:bg-rose-950/40' : 'border-slate-200 dark:border-slate-800 bg-slate-50/60 dark:bg-slate-950/60'}`}>
            <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
              {!singleDay ? (
                <span className={`rounded-full px-2.5 py-0.5 text-xs font-bold ${day.cancelled ? 'bg-rose-100 text-rose-700 line-through dark:bg-rose-500/20 dark:text-rose-300' : 'bg-indigo-100 text-indigo-700 dark:bg-indigo-500/20 dark:text-indigo-300'}`}>Day {i + 1}</span>
              ) : null}
            ...
            {day.cancelled && day.cancellationNote ? (
              <p className="mt-1.5 text-sm text-rose-700 dark:text-rose-300">{day.cancellationNote}</p>
            ) : null}
            ...
            {(day.facilitators ?? []).length ? (
              <div className="mt-2.5 flex flex-wrap gap-1.5">
                {(day.facilitators ?? []).map((person, j) => (
                  <span key={j} className="inline-flex items-center gap-1.5 rounded-full border border-indigo-100 bg-indigo-50/70 px-2.5 py-1 text-xs text-indigo-800 dark:border-indigo-900 dark:bg-indigo-950/60 dark:text-indigo-200">
                    <Mic className="h-3 w-3 shrink-0 text-indigo-500 dark:text-indigo-400" />
                    <span className="font-semibold">{person.name}</span>
                    {person.title ? <span className="text-indigo-500 dark:text-indigo-400">· {person.title}</span> : null}
                  </span>
                ))}
              </div>
            ) : null}
            {(day.sessions ?? []).length ? (
              <ul className="mt-2.5 space-y-1.5">
                {(day.sessions ?? [])
                  // A picked/registered track sees the shared spine + its own sessions only.
                  .filter((session) => !viewerSectionKey || !session.sectionKey || session.sectionKey === viewerSectionKey)
                  .map((session, j) => (
                  <li key={j} className="flex flex-wrap items-baseline gap-x-2 gap-y-0.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-3 py-2 text-sm">
                    {session.time ? (
                      <span className="inline-flex items-center gap-1 text-xs font-bold tabular-nums text-indigo-600 dark:text-indigo-400">
                    ...
                    {session.sectionKey && sectionName(session.sectionKey) ? (
                      <span className="rounded-full bg-emerald-50 px-2 py-0.5 text-[11px] font-semibold text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300">{sectionName(session.sectionKey)}</span>
                    ) : hasTracks ? (
                      <span className="rounded-full bg-slate-100 px-2 py-0.5 text-[11px] font-medium text-slate-500 dark:bg-slate-800 dark:text-slate-400">All tracks</span>
                    ) : null}
                    {session.venue ? (
              ...
              <ul className="mt-2.5 space-y-1.5">
                {day.features.map((feature, j) => (
                  <li key={j} className="flex items-start gap-2 text-sm text-slate-700 dark:text-slate-300">
                    <span className="mt-0.5 flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-emerald-50 dark:bg-emerald-950">
            ...
            {(daySpeakers[i + 1] ?? []).length ? (
              <div className="mt-3 border-t border-slate-200/70 dark:border-slate-800/70 pt-2.5">
