    ...
    <Section title="AI Event Assistant">
      ...
      {draft ? (
        <div className="mt-2 space-y-3 rounded-2xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900 p-4">
          <div>
            <p className="text-xs uppercase tracking-wide text-slate-400 dark:text-slate-500">Title</p>
            <p className="font-medium text-slate-900 dark:text-slate-100">{draft.title}</p>
          </div>
          <div>
            <p className="text-xs uppercase tracking-wide text-slate-400 dark:text-slate-500">Description</p>
            <p className="text-sm text-slate-700 dark:text-slate-300">{draft.description}</p>
          </div>
          {draft.agenda?.length ? (
            <div>
          ...
          {draft.audience ? (
            <div>
              <p className="text-xs uppercase tracking-wide text-slate-400 dark:text-slate-500">Audience</p>
              <p className="text-sm text-slate-700 dark:text-slate-300">{draft.audience}</p>
            </div>
          ) : null}
          <div className="flex items-center gap-3">
            <Button variant="primary" onClick={() => onApply(draft)}>Apply to form</Button>
            <span className="text-xs text-slate-400 dark:text-slate-500">{draft.source === 'ai' ? 'Generated by AI' : 'Template draft'}</span>
          </div>
        </div>
      ) : null}
    </Section>
