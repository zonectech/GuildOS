    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">Publish, manage attendance, and launch projector mode.</p>
        </div>
        <Link href={createHref} className="rounded-xl border border-slate-200 px-3 py-2 text-sm font-medium text-slate-700 transition hover:bg-slate-50">
          Create event
        </Link>
      </div>
      ...
      <div className="mt-4 space-y-3">
        {events.length ? (
          events.map((event) => (
            <article key={event.id} className="rounded-2xl border border-slate-200 p-4 transition hover:border-slate-300 hover:bg-slate-50/70">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                <div className="min-w-0">
                  <h3 className="truncate font-medium text-slate-950">{event.title}</h3>
                  <p className="mt-1 text-sm text-slate-500">{event.dateLabel} · {event.venue || 'Venue TBA'} · {event.communityName}</p>
                </div>
                <div className="flex flex-wrap items-center gap-2 text-sm text-slate-600">
                  <span className="rounded-full bg-slate-100 px-3 py-1">{event.registered} registered</span>
                  <span className={`rounded-full px-3 py-1 ${toneStyles[event.statusTone]}`}>{event.statusLabel}</span>
                </div>
              </div>

              <div className="mt-4 flex flex-wrap gap-2">
                <Link href={`/events/${event.slug}`} className="rounded-xl border border-slate-200 px-3 py-2 text-sm font-medium text-slate-700 transition hover:bg-slate-50">Manage</Link>
                <Link href="/dashboard/events/scanner" className="rounded-xl border border-slate-200 px-3 py-2 text-sm font-medium text-slate-700 transition hover:bg-slate-50">Check-in Scanner</Link>
