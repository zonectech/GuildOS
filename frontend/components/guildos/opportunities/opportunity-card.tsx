    ...
    <div className="flex h-full flex-col rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <Link href={`/opportunities/${opp.id}`} className="block truncate text-base font-semibold text-slate-950 hover:underline">{opp.title}</Link>
        ...
        {opp.matchScore !== null ? (
          <div className="shrink-0 text-right">
            ...
            <p className="mt-1 text-[10px] text-slate-400">{tier.label}</p>
          </div>
        ) : null}
      </div>

      {opp.matchReason ? (
        <p className="mt-3 text-sm text-slate-600">{opp.matchReason}</p>
      ) : opp.reasons.length ? (
        <p className="mt-3 text-sm text-slate-600">Why: {opp.reasons.slice(0, 2).join('; ')}.</p>
      ) : null}

      {opp.tags.length ? (
        <div className="mt-3 flex flex-wrap gap-1.5">
          {opp.tags.slice(0, 5).map((t) => <span key={t} className="rounded-full bg-slate-100 px-2 py-0.5 text-xs text-slate-600">{t}</span>)}
        </div>
      ) : null}

      <div className="mt-4 flex flex-1 flex-col justify-end gap-3">
        {deadline ? <p className="text-xs font-medium text-slate-500">{deadline}</p> : null}
        <div className="flex flex-wrap items-center gap-2">
          ...
          {ACTIONS.map((a) => (
            <button
              ...
              className={`rounded-xl border px-2.5 py-1.5 text-xs font-medium ${opp.action === a.value ? 'border-indigo-500 bg-indigo-50 text-indigo-700' : 'border-slate-200 text-slate-600 hover:bg-slate-50'}`}
            >
