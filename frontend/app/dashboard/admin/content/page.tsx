...
export default function ContentModerationPage() {
  ...
  return (
    <div className="space-y-5">
      <header>
        <h1 className="flex items-center gap-2 text-2xl font-semibold text-slate-950"><Flag className="h-6 w-6" /> Content moderation</h1>
        <p className="text-sm text-slate-500">Review posts and comments reported by the community. Remove content that breaks the rules or dismiss the reports.</p>
      </header>
      ...
      {posts.length ? (
        <section className="space-y-3">
          <h2 className="text-sm font-semibold uppercase tracking-wide text-slate-500">Reported posts ({posts.length})</h2>
          {posts.map((p) => (
            <div key={p.id} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <div className="flex shrink-0 gap-2">
                  ...
                  <button onClick={() => void actPost(p.id, 'dismiss')} className="inline-flex items-center gap-1.5 rounded-2xl border border-slate-300 px-4 py-2 text-sm font-medium text-slate-600"><Check className="h-4 w-4" /> Dismiss</button>
                </div>
              </div>
            </div>
          ))}
      ...
      {comments.length ? (
        <section className="space-y-3">
          <h2 className="text-sm font-semibold uppercase tracking-wide text-slate-500">Reported comments ({comments.length})</h2>
          {comments.map((c) => (
            <div key={c.id} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <div className="flex shrink-0 gap-2">
                  ...
                  <button onClick={() => void actComment(c.id, 'dismiss')} className="inline-flex items-center gap-1.5 rounded-2xl border border-slate-300 px-4 py-2 text-sm font-medium text-slate-600"><Check className="h-4 w-4" /> Dismiss</button>
                </div>
              </div>
            </div>
          ))}
