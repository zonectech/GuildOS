...
export default function AdminTicketsPage() {
  ...
  return (
    <div className="space-y-5">
      <header>
        <h1 className="text-2xl font-semibold text-slate-950">Ticket sales</h1>
        <p className="text-sm text-slate-500">Platform-wide paid-event economics: sales, commission, and organizer payouts.</p>
      </header>
      {loading ? (
        ...
        <div className="space-y-6">
          {totals ? (
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              <div className="rounded-2xl border border-slate-200 bg-white p-4">
                <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Tickets sold</p>
                <p className="mt-1 text-2xl font-bold text-slate-900">{totals.ticketsSold}</p>
                <p className="text-xs text-slate-500">Gross {ngn(totals.grossNgn)} (incl. {ngn(totals.gatewayFeesNgn)} gateway fees)</p>
              </div>
              ...
              <div className="rounded-2xl border border-slate-200 bg-white p-4">
                <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Owed to organizers</p>
                <p className="mt-1 text-2xl font-bold text-slate-900">{ngn(totals.owedToOrganizersNgn)}</p>
                <p className="text-xs text-slate-500">{ngn(totals.paidOutNgn)} already paid out</p>
              </div>
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <h2 className="flex items-center gap-2 text-sm font-bold text-slate-900"><Percent className="h-4 w-4 text-indigo-500" /> Commission rate</h2>
            <p className="mt-0.5 text-xs text-slate-500">Percentage of each ticket price GuildOS keeps (0–50). Applies to new sales only.</p>
            <div className="mt-3 flex items-center gap-2">
              <input type="number" min={0} max={50} value={commission} onChange={(e) => setCommission(e.target.value)} className="w-24 rounded-xl border border-slate-200 px-3 py-2 text-sm" />
            ...
            <div className="mt-5 border-t border-slate-100 pt-4">
              <h3 className="text-sm font-bold text-slate-900">Payout mode</h3>
              <p className="mt-0.5 text-xs text-slate-500">How organizer withdrawals are settled.</p>
              <div className="mt-2 flex gap-2">
                <button
                  onClick={() => void handlePayoutMode('MANUAL')}
                  disabled={savingMode}
                  className={`rounded-xl border px-3 py-1.5 text-xs font-semibold ${payoutMode === 'MANUAL' ? 'border-indigo-300 bg-indigo-50 text-indigo-700' : 'border-slate-200 bg-white text-slate-600'}`}
                >
                ...
                <button
                  ...
                  disabled={savingMode}
                  className={`rounded-xl border px-3 py-1.5 text-xs font-semibold ${payoutMode === 'AUTO' ? 'border-emerald-300 bg-emerald-50 text-emerald-700' : 'border-slate-200 bg-white text-slate-600'}`}
                >
                  ...
                  <div className="space-y-2">
                    {pendingPayouts.map((p) => (
                      <div key={p._id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-amber-200 bg-amber-50/60 px-4 py-3 text-sm">
                        <div>
                          <p className="font-semibold text-slate-900">{ngn(p.amountNgn)} — {p.communityName}</p>
                          <p className="text-xs text-slate-600">{p.bankName} · {p.accountNumber} · {p.accountName} · requested by {p.requestedByName} on {new Date(p.requestedAt).toLocaleDateString()}</p>
                        </div>
                        <div className="flex items-center gap-2">
                          ...
                          <button onClick={() => void handlePayout(p, 'REJECTED')} disabled={busyId === p._id} className="inline-flex items-center gap-1.5 rounded-xl border border-rose-300 bg-white px-3 py-1.5 text-xs font-semibold text-rose-600 disabled:opacity-50"><X className="h-3.5 w-3.5" /> Reject</button>
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="divide-y divide-slate-100">
                    {processedPayouts.map((p) => (
                      <div key={p._id} className="flex flex-wrap items-center justify-between gap-2 py-2.5 text-sm">
                        <div>
                          <p className="font-medium text-slate-800">{ngn(p.amountNgn)} — {p.communityName}</p>
                          <p className="text-xs text-slate-500">{p.bankName} · {p.accountName} · {p.processedAt ? new Date(p.processedAt).toLocaleDateString() : ''}</p>
                        </div>
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <h2 className="flex items-center gap-2 text-sm font-bold text-slate-900"><Ticket className="h-4 w-4 text-indigo-500" /> Sales by event</h2>
            {!events.length ? (
              <p className="mt-3 text-sm text-slate-500">No paid tickets sold yet.</p>
            ) : (
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {events.map((e) => (
                      <tr key={e.eventId}>
                        ...
                        <td className="py-2.5 pr-3 text-slate-700">{e.communityName}</td>
                        <td className="py-2.5 pr-3 text-slate-700">{ngn(e.ticketPriceNgn)}</td>
                        <td className="py-2.5 pr-3 font-semibold text-slate-900">{e.sold}</td>
                        <td className="py-2.5 pr-3 text-slate-700">{ngn(e.grossNgn)}</td>
                        <td className="py-2.5 pr-3 text-emerald-700">{ngn(e.commissionNgn)}</td>
                        <td className="py-2.5 pr-3 text-slate-700">{ngn(e.organizerNgn)}</td>
                        <td className="py-2.5 text-slate-500">{e.lastSaleAt ? new Date(e.lastSaleAt).toLocaleDateString() : '—'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
