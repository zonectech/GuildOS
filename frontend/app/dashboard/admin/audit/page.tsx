...
export default function AuditLogPage() {
  ...
  return (
    <div className="space-y-5">
      <header>
        <h1 className="flex items-center gap-2 text-2xl font-semibold text-slate-950"><ScrollText className="h-6 w-6" /> Admin audit log</h1>
        <p className="text-sm text-slate-500">Every administrator action — verifications, blocks, removals, suspensions, and broadcasts. {total} total.</p>
      </header>
      ...
      {loading ? (
              ...
              <li key={e.id} className="flex flex-wrap items-center gap-3 px-4 py-3">
                ...
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm text-slate-800">
                    ...
                    {e.targetType ? <span className="text-slate-500"> · {e.targetType.toLowerCase()} {e.targetId ? `#${e.targetId.slice(-6)}` : ''}</span> : null}
        ...
        <p className="rounded-2xl border border-dashed border-slate-300 p-8 text-center text-sm text-slate-500">No admin actions recorded yet.</p>
      )}

      {pages > 1 ? (
        <div className="flex items-center justify-between">
          <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page <= 1} className="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-700 disabled:opacity-50">Previous</button>
          <span className="text-sm text-slate-500">Page {page} of {pages}</span>
          <button onClick={() => setPage((p) => Math.min(pages, p + 1))} disabled={page >= pages} className="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-700 disabled:opacity-50">Next</button>
        </div>
      ) : null}
    </div>
  );
