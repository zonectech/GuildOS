...
export default function OpportunityModerationPage() {
  ...
  if (loading) {
    return <div className="rounded-3xl border border-slate-200 bg-white p-10 text-center text-slate-500 shadow-sm">Loading…</div>;
  }

  return (
    <div className="space-y-5">
      <header>
        <h1 className="text-2xl font-semibold text-slate-950">Opportunity moderation</h1>
        <p className="text-sm text-slate-500">Review pending and flagged opportunities before they reach students. Prevents scam postings.</p>
      </header>
      ...
      {queue.length ? (
            ...
            <div key={o.id} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <div className="flex shrink-0 gap-2">
                  ...
                  <button onClick={() => void decide(o.id, 'ARCHIVED')} className="rounded-2xl border border-slate-300 px-4 py-2 text-sm font-medium text-slate-600">Archive</button>
                </div>
              </div>
            </div>
          ))}
        ...
        <p className="rounded-2xl border border-dashed border-slate-300 p-6 text-sm text-slate-500">No opportunities awaiting review.</p>
      )}
    </div>
  );
}
