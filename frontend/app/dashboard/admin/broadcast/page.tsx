...
export default function BroadcastPage() {
  ...
  const inputCls = 'w-full rounded-xl border border-slate-200 dark:border-slate-800 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-200';

  return (
    <div className="space-y-5">
      <header>
        <h1 className="flex items-center gap-2 text-2xl font-semibold text-slate-950 dark:text-white"><Megaphone className="h-6 w-6" /> Send message</h1>
        <p className="text-sm text-slate-500 dark:text-slate-400">Message everyone, a role, or one specific user — via in-app notification and/or branded email. Delivery is instant.</p>
      </header>
      ...
      <div className="max-w-xl space-y-4 rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 shadow-sm">
        {/* Category */}
          ...
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
            {CATEGORIES.map((c) => (
              <button
                ...
                className={`rounded-xl border px-3 py-2 text-left transition ${category === c.value ? 'border-slate-900 ring-1 ring-slate-900' : 'border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800'}`}
              >
                <span className="block text-xs font-semibold text-slate-800 dark:text-slate-200">{c.label}</span>
                <span className="mt-0.5 block text-[11px] text-slate-500 dark:text-slate-400">{c.hint}</span>
              </button>
            ))}
          </div>
        </div>
        ...
        {/* Audience */}
        <div className="text-sm">
          <span className="mb-1.5 block font-medium text-slate-700 dark:text-slate-300">Audience</span>
          <div className="flex flex-wrap gap-2">
            {([['ALL', 'Everyone'], ['ROLE', 'A role'], ['USER', 'Specific user']] as const).map(([v, label]) => (
              <button
                ...
                type="button"
                onClick={() => setScope(v)}
                className={`rounded-xl border px-3 py-1.5 text-xs font-semibold transition ${scope === v ? 'border-slate-900 ring-1 ring-slate-900 text-slate-900 dark:text-slate-100' : 'border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800'}`}
              >
                {label}
              </button>
            ))}
        ...
        {/* Channels */}
        <div className="text-sm">
          <span className="mb-1.5 block font-medium text-slate-700 dark:text-slate-300">Channels</span>
          <div className="flex flex-wrap gap-2">
            <button
              type="button"
              onClick={() => setViaNotification((v) => !v)}
              className={`inline-flex items-center gap-1.5 rounded-xl border px-3 py-1.5 text-xs font-semibold transition ${viaNotification ? 'border-indigo-500 bg-indigo-50 text-indigo-700' : 'border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800'}`}
            >
            ...
            <button
              ...
              className={`inline-flex items-center gap-1.5 rounded-xl border px-3 py-1.5 text-xs font-semibold transition ${viaEmail ? 'border-indigo-500 bg-indigo-50 text-indigo-700' : 'border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800'}`}
            >
        ...
        <label className="block text-sm">
          <span className="mb-1 block font-medium text-slate-700 dark:text-slate-300">Message <span className="font-normal text-slate-400 dark:text-slate-500">(optional)</span></span>
        ...
        <label className="block text-sm">
          <span className="mb-1 block font-medium text-slate-700 dark:text-slate-300">Link / button <span className="font-normal text-slate-400 dark:text-slate-500">(optional, e.g. /events)</span></span>
