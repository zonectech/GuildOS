...
export default function AdminSponsorshipPage() {
  ...
  return (
    <div className="space-y-5">
      <header>
        <h1 className="text-2xl font-semibold text-slate-950">Sponsorship pipeline</h1>
        <p className="text-sm text-slate-500">Every sponsorship inquiry across the platform. Track deals and collect the platform fee.</p>
      </header>
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <p className="mt-1 text-xs text-slate-500">Organizers see these details when they mark a deal as won, so they know how much to remit and where.</p>
        <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <label className="block">
            <span className="mb-1 block text-xs font-medium text-slate-600">Fee percent (%)</span>
            <input
          ...
          <label className="block">
            <span className="mb-1 block text-xs font-medium text-slate-600">Bank name</span>
            <input
          ...
          <label className="block">
            <span className="mb-1 block text-xs font-medium text-slate-600">Account number</span>
            <input
        ...
        <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
          <div className="flex gap-4 text-xs">
            <span className="text-slate-500">Fees pending: <span className="font-semibold text-amber-700">{formatNaira(feeTotals.owed)}</span></span>
            <span className="text-slate-500">Fees collected: <span className="font-semibold text-emerald-700">{formatNaira(feeTotals.collected)}</span></span>
          </div>
          <button
          ...
          {settings.packageTemplates.map((tpl, index) => (
            <div key={index} className="rounded-2xl border border-slate-200 p-4">
              <input
                ...
                className="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-indigo-200"
              />
              <input
                value={tpl.price}
              ...
              <div className="mt-3 space-y-1.5">
                {SPONSOR_PERKS.map((perk) => (
                  <label key={perk.key} className="flex cursor-pointer items-center gap-2 text-xs text-slate-700">
      ...
      <div className="flex flex-wrap gap-2">
        {FILTERS.map((f) => (
          <button
            key={f}
            onClick={() => setFilter(f)}
            className={`rounded-full px-3.5 py-1.5 text-sm font-medium transition ${filter === f ? 'bg-slate-900 text-white' : 'border border-slate-200 bg-white text-slate-600 hover:bg-slate-50'}`}
          >
      ...
      {filtered.length ? (
        <div className="space-y-3">
          {filtered.map((q) => {
            ...
            return (
              <div key={q._id} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0">
                    <p className="font-semibold text-slate-950">{q.companyName}</p>
                    ...
                    <p className="mt-1 text-xs text-slate-400">
                      ...
                      {q.communityName ? ` · ${q.communityName}` : ''} · {new Date(q.createdAt).toLocaleString()}
                    </p>
                  </div>
                ...
                {q.message ? <p className="mt-3 whitespace-pre-line text-sm text-slate-600">{q.message}</p> : null}
                {q.status === 'WON' ? (
        ...
        <div className="rounded-3xl border border-dashed border-slate-300 bg-white p-10 text-center">
          ...
          <p className="mt-3 text-sm text-slate-500">No inquiries{filter !== 'ALL' ? ' with this status' : ' yet'}.</p>
        </div>
      )}
    </div>
  );
