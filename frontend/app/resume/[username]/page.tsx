  ...
  if (!data) return (
    <div className="min-h-screen bg-[#F4F6FA]"><StudentNav />
      <main className="mx-auto max-w-4xl px-4 py-10 space-y-4">
        {[1,2,3].map(i => <div key={i} className="h-32 animate-pulse rounded-3xl bg-white" />)}
      </main>
    </div>
  );
    ...
    <div className="min-h-screen bg-[#F4F6FA]">
      ...
      <main className="mx-auto max-w-3xl px-4 py-8">
        <div className="mb-4 flex items-center justify-between">
          <h1 className="text-xl font-bold text-slate-950">Resume</h1>
          <div className="flex gap-2">
            <button onClick={() => window.print()} className="no-print inline-flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3.5 py-2 text-sm font-medium text-slate-700 shadow-sm transition hover:bg-slate-50">
              <Download className="h-4 w-4" /> Print / Save PDF
            </button>
            <button onClick={() => void share()} className="no-print inline-flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3.5 py-2 text-sm font-medium text-slate-700 shadow-sm transition hover:bg-slate-50">
              <Share2 className="h-4 w-4" /> {copied ? 'Copied!' : 'Share'}
            </button>
          </div>
        </div>

        <div id="resume-doc" className="cv-document space-y-0 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-md print:rounded-none print:border-none print:shadow-none">
          {/* Header */}
          <div className="flex flex-col gap-5 border-b border-slate-100 p-8 sm:flex-row sm:items-start">
            ...
            <div className="flex-1">
              <div className="flex flex-wrap items-center gap-2">
                <h2 className="text-2xl font-extrabold tracking-tight text-slate-950">{resume.fullName}</h2>
              ...
              <p className="mt-2 max-w-xl text-sm leading-relaxed text-slate-600">{resume.summary}</p>
          ...
          <div className="grid divide-y divide-slate-100 sm:grid-cols-[1fr_220px] sm:divide-x sm:divide-y-0">
            ...
            <div className="space-y-6 p-8">
              ...
              {resume.showUniversity !== false ? (
                <section>
                  <h3 className="mb-3 flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-slate-500"><GraduationCap className="h-4 w-4" /> Education</h3>
                  <div className="rounded-2xl border border-slate-100 bg-slate-50 p-4">
                    <p className="font-bold text-slate-900">{resume.university || '—'}</p>
                    <p className="text-sm text-slate-600">{[resume.faculty, resume.department].filter(Boolean).join(' · ')}</p>
                    <p className="text-sm text-slate-500">{resume.level}{resume.graduationYear ? ` · Expected ${resume.graduationYear}` : ''}</p>
                  </div>
                </section>
              ) : null}
              ...
              {resume.showLeadership !== false && leadership.length ? (
                <section>
                  <h3 className="mb-3 flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-slate-500"><BriefcaseBusiness className="h-4 w-4" /> Leadership Experience</h3>
                  <div className="space-y-3">
                    {leadership.map(e => (
                      <div key={e.id} className="flex gap-3 rounded-2xl border border-slate-100 bg-slate-50 p-4">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-400">{new Date(e.startDate).toLocaleDateString(undefined, { year: 'numeric', month: 'short' })} – {e.endDate ? new Date(e.endDate).toLocaleDateString(undefined, { year: 'numeric', month: 'short' }) : 'Present'}</p>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              {resume.showCertificates !== false && verified.length ? (
                <section>
                  <h3 className="mb-3 flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-slate-500"><Award className="h-4 w-4" /> Verified Certificates</h3>
                  <div className="space-y-2">
                    {verified.map(c => (
                      <a key={c.serial} href={`/certificates/${c.serial}`} target="_blank" rel="noreferrer" className="flex items-start justify-between gap-3 rounded-2xl border border-slate-100 bg-slate-50 px-4 py-3 transition hover:border-indigo-200">
                        <div>
            ...
            <div className="space-y-6 p-6">
              ...
              {resume.skills?.length ? (
                <section>
                  <h3 className="mb-3 flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-slate-500"><BookOpen className="h-4 w-4" /> Skills & Interests</h3>
              ...
              {summary ? (
                <section>
                  <h3 className="mb-3 text-xs font-bold uppercase tracking-widest text-slate-500">Reputation</h3>
                  <div className="space-y-2">
                    {[
              ...
              {/* Verification link */}
              <section>
                <h3 className="mb-2 text-xs font-bold uppercase tracking-widest text-slate-500">Verify this resume</h3>
        ...
        <p className="mt-4 text-center text-xs text-slate-400">Generated by <strong>GuildOS</strong> · All credentials are verifiable on-chain at guildos.app</p>
      </main>
    </div>
  );
}
