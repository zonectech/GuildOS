...
4. **Meeting-link leak closed** — the "link unlocks at check-in" rule for online events was frontend-only; `GET /events/:slug` served `meetingLink` to any viewer and listings included it too. Now enforced server-side: only organizers and checked-in attendees receive it ([event-core.service.ts](backend/src/services/event/event-core.service.ts)), listings never include it, and the event page re-fetches after self check-in to obtain the unlocked link.
