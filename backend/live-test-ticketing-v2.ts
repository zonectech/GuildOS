...
async function main() {
  ...
  event.ticketTiers = [
    ...
    { name: 'VIP', price: 5000, capacity: 1 },
  ] as never;
  ...
  const order = await startTicketCheckout(eventId, String(buyer._id), { tierName: 'Early Bird', promoCode: 'FREE100', quantity: 3 });
  ...
  try {
    await startTicketCheckout(eventId, String(guest._id), { tierName: 'VIP', promoCode: 'HALF' });
